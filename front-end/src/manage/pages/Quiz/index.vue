<template>
    <el-form :model="quiz" :rules="rules" ref="quiz" label-width="120px" class="demo-quiz">
        ...
        <el-form-item required v-for="(cls, index) in quiz.classes" :label="index===0?'考生选择':''" :key="index" :prop="'classes.' + index + '.value'">
        ...
        <el-form-item required v-for="(pb, index) in quiz.score_value" :label="index===0?'分值设置':''" :key="index" :prop="'score_value.' + index + '.value'">
            <el-col :span="12">
                <el-select v-model="pb.scope" multiple placeholder="请选择试题类型">
                    <el-option :label="pbType" :value="pbType" v-for="(pbType,index) in types" :key="index"></el-option>
                </el-select>
            </el-col>
            <el-col :span="12">
                <el-input v-model='pb.value'></el-input>
            </el-col>
            <!-- <el-button @click.prevent="removeClass(cls)" v-if="index > 0">删除</el-button> -->
            <!-- <el-button @click="addClass" v-if="index === quiz.classes.length-1">添加班级</el-button> -->
        </el-form-item>
